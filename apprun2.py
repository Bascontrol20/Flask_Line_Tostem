...
### WebPage insert data complete, sampling message 1 msg, use threading in main program Update 08Dec2021
...
def plc_notify():
    try:
        ...
        sampling_timer = 5
        threading.Timer(sampling_timer,plc_notify).start()
    #MC01 M10000-M10009
        #MessageNo.1 M10000
        ...
        if device_batch_read[1] == 1:
            id= Linenotify.queryfilter_by(lineplc='M10001')[0].linetoken
...
if __name__ == '__main__':
    thr = threading.Thread(target=plc_notify)
    thr.start()
    app.run(debug=True)
